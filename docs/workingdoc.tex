...
In order to transition from the introductionary and theorteical discovery questions from the previous sections our team implemented multiple pipelines to select and transmit the raw data from the dataset.
 To address the first discovery question regarding the hidden assoications we wrote some python scripts. This script performs a keyword search across the "popular_shelves" attirbute of the `goodreads_books.json.gz` file. We spefically search for genres: 'romantasy', 'romantic-fantasy', 'fantasy-romance', 'romance', 'fantasy', and 'romantic'. The filtered results are stored in 01_filteredbooks.json. The resulting json consists of records: book_id, title, average_rating, shelves_list.
    In order to answer discovery question 2 we need to example the different interactions users make so we exmaine interactions on books across the site along with the types of reviews. We implemented a second filtering script that utilizes the book_id from our previous script to perform an extraction from the goodreads_reviews_dedup.json.gz dataset. The extracted data is stored in 03_extractedreviews.csv

    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{rating_histogram.png}
        \caption{ Distribution ofboos  rating in 01_goodreadsbooks.csv}
        \label{fig:placeholder}
    \end{figure}

    Examining the histogram of the average ratings of th
