...
In order to transition from the introductory and theorteical discovery questions from the previous sections our team implemented multiple scripts to select and gather the raw data from the dataset.
 To address the first discovery question regarding the hidden associations we wrote some python scripts to filter the massive dataset. These scripts took a while to run our computer. First we performed a keyword search across the "popular\_shelves" attirbute of the \texttt{goodreads\_books.json.gz} file. We spefically search for genres: 'romantasy', 'romantic-fantasy', 'fantasy-romance', 'romance', 'fantasy', and 'romantic'. The filtered results are stored in \texttt{01\_filteredbooks.json}. The resulting json consists of records: book\_id, title, average\_rating, shelves\_list.


To answer discovery question 2, we need to examine the different user interactions, so we implemented a second filtering script utilizing the book\_id from our previous script to extract reviews from the \texttt{goodreads\_reviews\_dedup.json.gz} dataset, storing the output in \texttt{03\_extractedreviews.csv}.

The datasets required significant cleaning before our team could conduct meaningful mining. First, we removed generic administrative tags like `to-read' and `ebooks' from the books dataset to reduce noise. Second, to focus our analysis and ensure data comprehension, we removed any non-English records from the reviews dataset using the Python package \texttt{langid}.
...
Before applying our mining algorithms, our team examined the distributions of ratings, community-defined tags (for example users can create virtual bookshelves and give them a name), and review engagement to establish a baseline understanding of the ``Romantasy'' and other romance genres subset.
 ...
 \begin{figure}[htbp]
    ...
    \caption{Distribution of boos rating in 01\_goodreadsbooks.csv}
...
In order to answer discovery question 1 we need to example the different interactions users make so we examine interactions on books across the site along with the types of reviews. We implemented a second filtering script that utilizes the book\_id from our previous script to perform an extraction from the \texttt{goodreads\_reviews\_dedup.json.gz} dataset. The extracted data is stored in \texttt{03\_extractedreviews.csv}.
...
While we did not see strong associations between romance genres at our current minimum support threshold of 0.01, our findings partially support the broader hypothesis of sub-genre cross-pollination. Specifically, we demonstrated that readers of fantasy are highly likely to read and tag other closely related speculative fiction genres. This is evidenced by the strong associations found between ``paranormal'' and ``supernatural'' fiction, as well as the heavy overlap between fantasy and science fiction readers.
