...
In order to transition from the introductionary and theorteical discovery questions from the previous sections our team implemented multiple pipelines to select and transmit the raw data from the dataset.
 To address the first discovery question regarding the hidden assoications we wrote some python scripts. This script performs a keyword search across the "popular\_shelves" attirbute of the \texttt{goodreads\_books.json.gz} file. We spefically search for genres: 'romantasy', 'romantic-fantasy', 'fantasy-romance', 'romance', 'fantasy', and 'romantic'. The filtered results are stored in \texttt{01\_filteredbooks.json}. The resulting json consists of records: book\_id, title, average\_rating, shelves\_list.

In order to answer discovery question 2 we need to example the different interactions users make so we exmaine interactions on books across the site along with the types of reviews. We implemented a second filtering script that utilizes the book\_id from our previous script to perform an extraction from the \texttt{goodreads\_reviews\_dedup.json.gz} dataset. The extracted data is stored in \texttt{03\_extractedreviews.csv}.
...
\end{figure}
    ...
    Examining the histogram of the average ratings of romance books in the 01_goodreadsbooks.csv we find that most readers rated the books around 4. This suggests that romance readers find books great reads. They often are not critical of books.
...
Analysis of the shelves_list indicates that the tag 'to-read' is the most frequent attribute. During the preprocessing phase, we implemented a filter to only show tags that correspond to actually refer to book descriptions and not generic tags such as "to-read". However after during the postprocessing phase the most prevalent tags included fiction, romance, and fantasy. This aligns with the "Romatansy" genre which confirms Romantasy is the most pollinated genre.
